chrome.extension.sendMessage({}, function (response) {
    ...
    InboxSDK.load('1', 'sdk_rails-gmail_aa430103a9').then(function (sdk) {
        ...
        function shouldUpdateNavItem(array_of_navItems, todo_object) {
            /**
             This function iterates through the array of navitems looking for a nav item that has the same property has the todo list item given to is
            ...
            if (array_of_navItems.length > 0) {
                const item = _.find(array_of_navItems,{'description':todo_object.description});
                if (item.checked != todo_object.checked) {
        ...
        sdk.Lists.registerThreadRowViewHandler(function (threadRowView) {
            ...
            var threadBtnStream = Kefir.stream(function (inEmitter) {
                ...
                return function () {
                }; //we need to return a function that gets called when the stream ends
            });
                                    ...
                                    if (!isTodo) {
                                        if (threadRowView.getThreadID() == _threadID) {
                                            $.ajax({
                                                ...
                                                data: {
                                                    todo: {
                                                        item: threadRowView.getSubject(),
                                                        checked: false,
                                                        description: _threadID
                                                    }
                                                },
                                                success: function (response) {
            ...
            threadBtnEmmiter.emit(threadViewObj);

           Array.observe(array_of_todos, function (changes) {
            var _threadID = threadRowView._threadRowViewDriver._cachedThreadID;
            var thread = _.find(array_of_todos,  {'description' : _threadID});
               if (thread.checked == true) {
                   ...
                   threadViewObj.iconUrl = "http://pontifolio.com/img/green-image.jpg";
                   threadBtnEmmiter.emit(threadViewObj)

               } else {
                   threadBtnEmmiter.emit(null);
                   threadViewObj.iconUrl  = "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcR6t41ErQxx0y1rApv207bM3LznQVdvOILrYy-XTUVg3JpxGvRn"
                   threadBtnEmmiter.emit(threadViewObj)
